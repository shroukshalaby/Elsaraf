...
class UserController extends Controller
{
    ...
    public function fundVirtual(Request $request)
    {
        ...
        if ($user->balance > $request->amount) {
            ...
            $createCard = $card->fundCard($data);
            $getCard = $card->getCard($data2);
            $result = $createCard;
            $result2 = $getCard;
            if (array_key_exists('data', $result) && ($result['status'] === 'success')) {
                ...
                $charge['user_id'] = $user->id;
                $charge['ref_id'] = $trx;
                $charge['amount'] = $request->amount * $set->virtual_charge / 100;
                $charge['log'] = 'Virtual Card funding charge #';
                Charges::create($charge);
                $his['user_id'] = $user->id;
                $his['amount'] = $request->amount + ($request->amount * $set->virtual_createcharge / 100);
                $his['ref'] = $trx;
                $his['main'] = 0;
                $his['type'] = 2;
                History::create($his);
                ...
                //Debit User
                $user->balance = $user->balance - $request->amount - ($request->amount * $set->virtual_charge / 100);
                $user->save();
                $vcard->amount = $result2['data']['amount'];
                if ($result2['data']['is_active'] == false) {
                    $vcard->status = 0;
                }
                $vcard->save();
                return redirect()->route('transactions.virtual', ['id' => $vcard->id])->with('success', 'Virtual card was successfully funded');
            } else {
    ...
    public function createVirtual(Request $request)
    {
        ...
        if ($user->balance > $request->amount) {
            ...
            $data = array(
                ...
                "callback_url" => route('use.virtual')
            );
            $card = new VirtualCard();
            $createCard = $card->createCard($data);
            $result = $createCard;
            if (array_key_exists('data', $result) && ($result['status'] === 'success')) {
                ...
                $charge['user_id'] = $user->id;
                $charge['ref_id'] = $trx;
                $charge['amount'] = $request->amount * $set->virtual_createcharge / 100;
                ...
                //Debit User
                $user->balance = $user->balance - $request->amount - ($request->amount * $set->virtual_createcharge / 100);
                $user->save();
                ...
                $sav['amount'] = $request->amount;
                $sav['charge'] = $request->amount * $set->virtual_createcharge / 100;
                Virtual::create($sav);
    ...
    public function transactionsVirtual($id)
    {
        ...
        $data['log'] = Virtualtransactions::wherevirtual_id($id)->orderBy('created_at', 'DESC')->get();
    ...
    public function terminateVirtual($id)
    {
        ...
        $card = new VirtualCard();
        $terminateCard = $card->terminateCard($data);
        $getCard = $card->getCard($data2);
        $result = $terminateCard;
        $result2 = $getCard;
        if (array_key_exists('data', $result) && ($result['status'] === 'success')) {
            //Debit User
            ...
            Audit::create($audit);
            return back()->with('success', 'Virtual card was successfully terminated');
        } else {
            return back()->with('alert', 'An Error Occured, Try again Later');
        }
    }
    ...
    public function submitbill(Request $request)
    {
        ...
        if ($user->balance > $request->amount) {
            ...
            if (array_key_exists('data', $result) && ($result['status'] === 'success')) {
                ...
                $charge['user_id'] = $user->id;
                $charge['ref_id'] = $trx;
                $charge['amount'] = $request->amount * $set->bill_charge / 100;
                $charge['log'] = 'Bill payment for #' . $trx;
                Charges::create($charge);

                $his['user_id'] = $user->id;
                $his['amount'] = $request->amount + ($request->amount * $set->bill_charge / 100);
                $his['ref'] = $trx;
                $his['main'] = 1;
                $his['type'] = 2;
                History::create($his);
                ...
                //Debit User
                $user->balance = $user->balance - $request->amount - ($request->amount * $set->bill_charge / 100);
                $user->save();
                ...
                $sav['amount'] = $request->amount;
                $sav['biller'] = $ebiller;
                $sav['charge'] = $request->amount * $set->bill_charge / 100;
                $sav['ref'] = $trx;
    ...
    public function submitticket(Request $request)
    {
        ...
        $sav['ticket_id'] = $token = Str::random(16);
        $sav['status'] = 0;
        Ticket::create($sav);
        if ($set['email_notify'] == 1) {
    ...
    public function storefront()
    {
        ...
        $data['store'] = Storefront::whereUser_id(Auth::guard('user')->user()->id)->orderby('id', 'desc')->get();
        $data['product'] = Product::whereUser_id(Auth::guard('user')->user()->id)->orderby('id', 'desc')->get();
        $data['category'] = Productcategory::whereUser_id(Auth::guard('user')->user()->id)->get();
        $data['received'] = Order::whereStatus(1)->whereseller_id(Auth::guard('user')->user()->id)->sum('total');
        $data['total'] = Order::whereseller_id(Auth::guard('user')->user()->id)->sum('total');
    ...
    public function submitproduct(Request $request)
    {
        $user = $data['user'] = User::find(Auth::guard('user')->user()->id);
        $sav['user_id'] = Auth::guard('user')->user()->id;
        $sav['ref_id'] = $trx = 'PRD-' . Str::random(6);
    ...
    public function acquireproduct(Request $request)
    {
        ...
        if ($request->type == 'card') {
            ...
            $this->validate($request, ['number' => 'required', 'expiry' => 'required', 'cvc' => 'required',]);
            $exp = $request->expiry;
            ...
            try {
                $token = Token::create(array("card" => array("number" => $request->number, "exp_month" => $emo, "exp_year" => $eyr, "cvc" => $request->cvc)));
                try {
                    ...
                    if ($charge['status'] == 'succeeded') {
                        ...
                        $his['amount'] = $total - ($total * $set->product_charge / 100);
                        $his['ref'] = $token;
                        $his['main'] = 1;
                        $his['type'] = 1;
                        History::create($his);
                ...
                try {
                    ...
                    if ($charge['status'] == 'succeeded') {
                        ...
                        foreach ($cart as $checkout) {
                            ...
                            $sav['charge'] = $charge = ($checkout->total * $set->product_charge / 100);
                        ...
                        $merchant->revenue = $merchant->revenue + $total - ($total * $set->product_charge / 100);
                        $merchant->save();
                        //Charges
                        $chargex['user_id'] = $up_mer->id;
    ...
    public function Paidinvoice($id)
    {
        ...
        if ($up_mer->balance > $charge || $up_mer->balance == $charge) {
            $up_mer->balance = $data->total * $set->invoice_charge / 100;
            $up_mer->save();
            $data->status = 1;
            ...
            $his['amount'] = $data->total * $set->invoice_charge / 100;
            $his['ref'] = $data->ref_id;
            $his['main'] = 0;
            $his['type'] = 1;
            History::create($his);
            ...
            try {
                ...
                try {
                    ...
                    if ($charge['status'] == 'succeeded') {
                        ...
                        $chargex['amount'] = $ext->total * $set->invoice_charge / 100;
                        $chargex['log'] = 'Charges for invoice #' . $request->link;
                        Charges::create($chargex);
                        $his['user_id'] = $merchant->user_id;
                        $his['amount'] = $ext->total - ($ext->total * $set->invoice_charge / 100);
                        $his['ref'] = $xtoken;
                        $his['main'] = 0;
                        $his['type'] = 1;
                        History::create($his);
                        ...
                        return back()->with('success', 'Invoice was successfully paid');
                    }
                } catch (\Stripe\Exception\CardException $e) {
                    return back()->with('alert', $e->getMessage());
                }
    ...
    {
        $data['title'] = 'Merchant';
        $data['merchant'] = Merchant::whereUser_id(Auth::guard('user')->user()->id)->get();
        ...
        if ($ext->status == 0) {
            if ($request->type == 'card') {
                ...
                try {
                    ...
                    try {
                        ...
                        if ($charge['status'] == 'succeeded') {
                            ...
                            $sav['type'] = 4;
                            $sav['amount'] = $ext->quantity * $ext->amount * $set->merchant_charge / 100;
                            $sav['card_number'] = $request->number;
                            $sav['payment_type'] = 'card';
                            $sav['ip_address'] = user_ip();
                            ...
                            $chargex['amount'] = $ext->quantity * $ext->amount * $set->merchant_charge / 100;
                            $chargex['log'] = 'Charges for merchant payment #' . $request->link;
                            Charges::create($chargex);
                            $his['user_id'] = $merchant->user_id;
                            $his['amount'] = $ext->quantity * $ext->amount * $set->merchant_charge / 100;
                ...
                if ($amount < $debit->balance || $amount == $debit->balance) {
                    $merchant = Merchant::whereMerchant_key($ext->merchant_key)->first();
                    $up_mer = User::whereId($merchant->user_id)->first();
                    $up_mer->balance = $up_mer->balance + ($ext->quantity * $ext->amount - ($ext->quantity * $ext->amount * $set->merchant_charge / 100));
                    $up_mer->save();
                    $debit->balance = $debit->balance - ($ext->amount * $ext->quantity);
                    $debit->save();
                    $ext->status = 1;
                    $ext->charge = $ext->amount * $ext->quantity * $set->merchant_charge / 100;
                    $ext->save();
                    //Transaction History
                    $sav['ref_id'] = $ext->reference;
                    $sav['type'] = 4;
                    $sav['amount'] = $ext->amount * $ext->quantity * $set->merchant_charge / 100;
                    $sav['sender_id'] = Auth::guard('user')->user()->id;
                    $sav['receiver_id'] = $merchant->user_id;
                    $sav['payment_link'] = $ext->id;
                    $sav['payment_type'] = 'account';
                    ...
                    $chargex['user_id'] = $merchant->user_id;
                    $chargex['ref_id'] = $ext->reference;
                    $chargex['amount'] = $ext->amount * $ext->quantity * $set->merchant_charge / 100;
                    $chargex['log'] = 'Charges for merchant payment #' . $request->link;
                    Charges::create($chargex);
                    $his['user_id'] = $merchant->user_id;
                    $his['amount'] = $ext->amount * $ext->quantity * $set->merchant_charge / 100;
                    $his['ref'] = $ext->reference;
                    $his['main'] = 0;
                    $his['type'] = 1;
                    History::create($his);
    ...
    public function submitmerchant(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $location = 'asset/profile/' . $filename;
            Image::make($image)->save($location);
            $trx = 'MER-' . Str::random(6);
            $sav = [
                'user_id' => Auth::guard('user')->user()->id,
                'merchant_key' => Str::random(16),
                'api_token' => md5((Str::random(16) . microtime())),
                'notification_secret' => md5((Str::random(16) . microtime())),
                'site_url' => $request->site_link,
                'image' => $filename,
                'name' => $request->merchant_name,
                'description' => $request->merchant_description,
                'email' => $request->email,
                'ref_id' => $trx,
                'status' => 1,
            ];
    ...
    public function card(Request $request)
    {
        ...
        try {
            $token = Token::create(array("card" => array("number" => $request->number, "exp_month" => $emo, "exp_year" => $eyr, "cvc" => $request->cvc)));
            try {
                $charge = Charge::create(array('card' => $token['id'], 'currency' => $currency->name, 'amount' => $cnts * 100, 'description' => $set->site_name . 'funding',));
                if ($charge['status'] == 'succeeded') {
    ...
    {
        return view('user.profile.distributions');
    }
    public function affiliate()
    {
        return view('user.profile.affiliate');
    }
    ...
    public function withdrawsubmit(Request $request)
    {
        ...
        $amount = $request->amount + (($request->amount * $set->withdraw_charge / 100) * $rl);
        $token = 'ST-' . Str::random(6);
            ...
            if ($user->business_level == 1) {
                ...
                if ($total < $set->withdraw_limit || $set->withdraw_limit == $total) {
                    ...
                    if ($t_sub > $request->amount) {
                        ...
                        $sav['user_id'] = Auth::guard('user')->user()->id;
                        $sav['reference'] = $token;
                        $sav['amount'] = $request->amount - $t_sub;
                        $sav['charge'] = ($request->amount * $set->withdraw_charge / 100) * $rl;
                        $sav['status'] = 0;
                        $sav['type'] = 1;
                        $sav['bank_id'] = $bank->id;
                        ...
                        $charge['user_id'] = $user->id;
                        $charge['ref_id'] = $token;
                        $charge['amount'] = ($request->amount * $set->withdraw_charge / 100) * $rl;
    ...
    public function ownbank()
    {
        ...
        $data['adminbank'] = Adminbank::whereId(1)->first();
        $data['transfer'] = Transfer::wheresender_id(Auth::guard('user')->user()->id)->orwhere('receiver_id', Auth::guard('user')->user()->id)->orderby('id', 'desc')->paginate(6);
        $data['received'] = Transfer::where('Temp', Auth::guard('user')->user()->email)->latest()->get();
            ...
            if ($user->balance > $amountx || $user->balance == $amountx) {
                ...
                if (count($check) > 0) {
                    if ($user->status == 0) {
                        ...
                        Charges::create($charge);
                        $his['user_id'] = $user->id;
                        $his['amount'] = $request->amount + ($request->amount * $set->transfer_charge / 100);
                        $his['ref'] = $token;
                        $his['main'] = 1;
                        $his['type'] = 2;
                        History::create($his);
    ...
    public function submitrequest(Request $request)
    {
        ...
        $token = 'RQ-' . Str::random(6);
        if ($user->email != $request->email) {
            if (count($check) > 0) {
                $sav['ref_id'] = $token;
                $sav['confirm'] = Str::random(8);
                $sav['amount'] = $request->amount;
                $sav['charge'] = $request->amount * $set->transfer_charge / 100;
                $sav['email'] = $request->email;
                $sav['user_id'] = Auth::guard('user')->user()->id;
            ...
            try {
                ...
                try {
                    ...
                    if ($charge['status'] == 'succeeded') {
                        ...
                        $his['amount'] = $request->amount - ($request->amount * $set->single_charge / 100);
                        $his['ref'] = $xtoken;
                        $his['main'] = 0;
                        $his['type'] = 1;
                        History::create($his);
                ...
                try {
                    ...
                    try {
                        ...
                        if ($charge['status'] == 'succeeded') {
                            ...
                            Charges::create($charges);
                            $his['user_id'] = $receiver->id;
                            $his['amount'] = $request->amount - ($request->amount * $set->donation_charge / 100);
                            $his['ref'] = $xtoken;
                            $his['main'] = 0;
                            $his['type'] = 1;
                            History::create($his);
    ...
    {
        $data['title'] = "Profile";
        $data['country'] = Countrysupported::all();
    ...
    public function submitcompliance(Request $request)
    {
        ...
        if ($request->hasFile('idcard')) {
            ...
            $filename = 'idcard' . time() . $image->extension();
            $location = 'asset/profile/' . $filename;
            if ($com->idcard != null) {
                $path = './asset/profile/';
                $link = $path . $user->idcard;
                if (file_exists($link)) {
                    @unlink($link);
                }
            }
        ...
        if (count($check) > 0) {
            if ($key->expiring_date > Carbon::now()) {
                ...
                if ($user->balance > $request->amount || $user->balance == $request->amount) {
                    ...
                    Charges::create($charge);
                    $his['user_id'] = $receiver->id;
                    $his['amount'] = $request->amount - ($request->amount * $set->subscription_charge / 100);
                    $his['ref'] = $xtoken;
                    $his['main'] = 0;
                    $his['type'] = 1;
                    History::create($his);
    ...
    public function transactions()
    {
        ...
        $data['single'] = Transactions::wheresender_id($user)->wheretype(1)->orWhere('receiver_id', $user)->orwhere('type', 1)->latest()->get();
        $data['donation'] = Transactions::wheresender_id($user)->wheretype(2)->orWhere('receiver_id', $user)->orwhere('type', 2)->latest()->get();
        $data['invoice'] = Transactions::wheresender_id($user)->wheretype(3)->orWhere('receiver_id', $user)->orwhere('type', 3)->latest()->get();
