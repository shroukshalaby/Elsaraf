...
<div class="container-fluid mt--6">
  <div class="content-wrapper">
    <div class="row align-items-center py-4">
      <div class="col-4">
        <h6 class="h2 d-inline-block mb-0">{{__('Donation')}}</h6>
      </div>
      <div class="col-8 text-right">
        <a data-toggle="modal" data-target="#donation-page" href="" class="btn btn-sm btn-neutral"><i class="fal fa-plus"></i> {{__('Create Payment Link')}}</a>
      </div>
    </div>
    <div class="row">
      <div class="col-md-12">
              ...
              <div class="modal-body">
                <form action="{{route('submit.donationpage')}}" enctype="multipart/form-data" method="post" id="modal-detailx">
                  @csrf
                    ...
                    <div class="form-group row">
                      <div class="col-lg-12">
                          <textarea type="text" name="description" placeholder="{{__('Description')}}" rows="4" class="form-control"></textarea>
                      </div>
                    </div>
                    <div class="text-right">
                    <button type="submit" class="btn btn-neutral btn-block" form="modal-detailx">{{__('Create link')}}</button>
                    </div>
                </form>
              </div>
            </div>
        ...
        <div class="row">
                      ...
                      <div class="row mb-2">
                        ...
                        <div class="col-8 text-right">
                          ...
                          <div class="dropdown-menu dropdown-menu-left">
                            @if($val->active==1)
                                <a class='dropdown-item' href="{{route('dplinks.unpublish', ['id' => $val->id])}}"><i class="fal fa-ban"></i>{{ __('Disable')}}</a>
                            @else
                                <a class='dropdown-item' href="{{route('dplinks.publish', ['id' => $val->id])}}"><i class="fal fa-check"></i>{{ __('Activate')}}</a>
                            @endif
                            <a class="dropdown-item" href="{{route('user.sclinkstrans', ['id' => $val->id])}}"><i class="fal fa-sync"></i>{{__('Transactions')}}</a>
                            <a class="dropdown-item" data-toggle="modal" data-target="#donors{{$val->id}}" href="#"><i class="fal fa-user"></i>{{__('Donors')}}</a>
                          ...
                          <div class="modal-body p-0">
                              <div class="card bg-white border-0 mb-0">
                                  <div class="card-header">
                                    <h3 class="mb-1 font-weight-bolder">{{__('Delete Payment link')}}</h3>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                      <span aria-hidden="true">&times;</span>
                                    </button>
          ...
          <div class="col-md-12 mb-5">
              <div class="text-center mt-8">
                ...
                <p class="text-dark text-sm card-text">We couldn't find any donation page to this account</p>
              </div>
            </div>
          @endif
        </div>
