              ...
              <div class="modal-body">
                <form action="{{route('withdraw.submit')}}" method="post">
                  ...
                  <div class="form-group row">
                    <div class="col-lg-12">
                      <div class="input-group">
                        ...
                        <input type="number" name="amount" class="form-control" placeholder="0.00" max="{{$user->balance}}" required>
                      </div>
                      <span class="form-text text-xs">Withdraw charge is {{$set->withdraw_charge}}%, 
                        @if($user->business_level==1)
                        & maximum withdrawal is {{$currency->symbol.number_format($set->withdraw_limit)}} for startup business. 
                        @elseif($user->business_level==2)
                        & maximum withdrawal is {{$currency->symbol.number_format($set->starter_limit)}} for verified business.
                  ...
                  <div class="text-right">
                    <button type="submit" class="btn btn-neutral btn-block"><span id="rex"></span>{{__('Save')}}</button>
                  </div>
                </form>
              </div>
            </div>
                ...
                <div class="card-body">
                  ...
                  <div class="row">
                      <div class="col">
                        ...
                        @if($val->type==2)
                          <span class="badge badge-pill badge-primary"><i class="fal fa-user"></i> Sub Account</span>                        
                        @elseif($val->type==1)
                          <span class="badge badge-pill badge-primary"><i class="fal fa-user"></i> Main </span>
                        @endif                        
                        @if($val->status==1)
                          @if($val->type==1)
                            <span class="badge badge-pill badge-primary">{{__('Charge')}}: {{$currency->symbol.$val->charge}}</span>
                          @endif
                        @endif
                        @if($val->status==1)
                        ...
                        @elseif($val->status==2)
                          <span class="badge badge-pill badge-info"><i class="fal fa-close"></i> {{__('Declined')}}</span>
                        @endif
                      </div>
                    </div>
                </div>
